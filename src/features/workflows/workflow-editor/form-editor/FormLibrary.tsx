...
const UserSearchPreview = ({
  ...
  // Sync searchQuery with current value if it exists on mount / when value changes externally (during render-time)
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (val.length >= 3) {
      ...
      if (selectedUser && selectedUser.name === val) {
        setResults([]);
        setIsSearching(false);
        return;
      }
...
const LocalFormVisibilityOptions = ({
  selectedForm,
  index,
  t
}: {
...
const OtherFormsVisibilityOptions = ({
  q,
  forms,
  selectedForm,
  getFormRank,
  showOtherFormsForQuestions,
  t
}: {
  q: FormQuestion;
  forms: Form[];
  selectedForm: Form;
  getFormRank: (formId: string) => number;
  showOtherFormsForQuestions: Set<string>;
  t: TFunction;
}) => {
  const localQuestionIds = React.useMemo(() => new Set(selectedForm.questions.map(sq => sq.id)), [selectedForm.questions]);
  ...
  const otherAvailableForms = React.useMemo(() =>
    forms.filter(f => f.id !== selectedForm.id && getFormRank(f.id) < getFormRank(selectedForm.id)),
    [forms, selectedForm.id, getFormRank]
  );
  const hasOtherFormsWithDropdowns = React.useMemo(() =>
    otherAvailableForms.some(f => f.questions.some(fq => fq.type === 'dropdown')),
    [otherAvailableForms]
  );
...
const DependencyConditionSelector = ({
  q,
  forms,
  ...
  q: FormQuestion;
  forms: Form[];
  handleQuestionUpdate: (questionId: string, updates: Partial<FormQuestion>) => void;
  t: TFunction;
}) => {
...
const PreviewCheckboxGroup = ({
  q,
  previewAnswers,
  setPreviewAnswers
}: {
  q: FormQuestion;
  previewAnswers: Record<string, string | string[] | number | undefined>;
  setPreviewAnswers: React.Dispatch<React.SetStateAction<Record<string, string | string[] | number | undefined>>>;
}) => {
  const checkedArray = Array.isArray(previewAnswers[q.id]) ? (previewAnswers[q.id] as string[]) : [];
  ...
  const handleCheckboxChange = (opt: string, isChecked: boolean) => {
    setPreviewAnswers(prev => {
      ...
      return {
        ...prev,
        [q.id]: isChecked ? [...current, opt] : current.filter((x) => x !== opt)
      };
    });
  };
...
interface QuestionEditorCardProps {
  ...
  forms: Form[];
  showOtherFormsForQuestions: Set<string>;
  setShowOtherFormsForQuestions: React.Dispatch<React.SetStateAction<Set<string>>>;
  getFormRank: (formId: string) => number;
  handleToggleCollapse: (questionId: string) => void;
  handleDeleteQuestion: (questionId: string) => void;
  handleMoveQuestion: (questionId: string, direction: 'up' | 'down') => void;
  isDuplicateQuestionLabel: (label: string, questionId: string) => boolean;
  handleQuestionUpdate: (questionId: string, updates: Partial<FormQuestion>) => void;
  setAlternativeWarning: React.Dispatch<React.SetStateAction<{ title: string; message: string } | null>>;
  t: TFunction;
}

interface DisclaimerQuestionCardProps {
  q: FormQuestion;
  index: number;
  questionNumberMap: Map<string, string>;
...
const DisclaimerQuestionCard = ({
  ...
  handleMoveQuestion,
  isDuplicateQuestionLabel,
  handleQuestionUpdate,
  t
}: DisclaimerQuestionCardProps) => {
  return (
    <div className="question-editor-card disclaimer-card" data-q-id={q.id}>
...
const StandardQuestionCard = ({
  ...
  return (
    <div className={`question-editor-card ${q.dependencyQuestion ? 'conditional-card' : ''}`} data-q-id={q.id}>
      <div className="card-header" style={{ alignItems: 'center' }}>
        ...
        {(index > 0 || forms.length > 1) && (
          <div className="field-group" style={{ display: 'block', width: '100%' }}>
            ...
            >
              ...
              <LocalFormVisibilityOptions
                selectedForm={selectedForm}
                index={index}
                t={t}
              />
              
              <OtherFormsVisibilityOptions
                q={q}
                forms={forms}
                selectedForm={selectedForm}
                getFormRank={getFormRank}
                showOtherFormsForQuestions={showOtherFormsForQuestions}
                t={t}
              />
            </select>

            {q.dependencyQuestion && (
              <DependencyConditionSelector
                q={q}
                forms={forms}
...
const QuestionEditorCard = ({
  ...
  if (q.type === 'disclaimer') {
    return (
      <DisclaimerQuestionCard
        ...
        isDuplicateQuestionLabel={isDuplicateQuestionLabel}
        handleQuestionUpdate={handleQuestionUpdate}
        t={t}
      />
    );
  ...
  return (
    <StandardQuestionCard
      ...
      selectedForm={selectedForm}
      showOtherFormsForQuestions={showOtherFormsForQuestions}
      setShowOtherFormsForQuestions={setShowOtherFormsForQuestions}
      getFormRank={getFormRank}
      handleToggleCollapse={handleToggleCollapse}
      handleDeleteQuestion={handleDeleteQuestion}
      handleMoveQuestion={handleMoveQuestion}
      isDuplicateQuestionLabel={isDuplicateQuestionLabel}
      handleQuestionUpdate={handleQuestionUpdate}
      setAlternativeWarning={setAlternativeWarning}
      t={t}
    />
  );
};

interface FormLibrarySidebarProps {
  forms: Form[];
...
interface SimulationDependenciesCardProps {
  externalDependencies: { form: Form; question: FormQuestion }[];
  previewAnswers: Record<string, string | string[] | number | undefined>;
  setPreviewAnswers: React.Dispatch<React.SetStateAction<Record<string, string | string[] | number | undefined>>>;
  t: TFunction;
}

const SimulationDependenciesCard = ({
  externalDependencies,
  previewAnswers,
  setPreviewAnswers,
  t
}: SimulationDependenciesCardProps) => {
  return (
...
const QuestionPreviewItem = ({
  ...
  if (q.type === 'disclaimer') {
    return (
      <div className="preview-disclaimer" style={{ marginBottom: 'var(--spacing-xl)', borderBottom: '1px solid var(--panel-border)', paddingBottom: 'var(--spacing-md)' }}>
...
interface DeleteQuestionConfirmModalProps {
  questionToDelete: FormQuestion;
  questionToDeleteDeps: { formTitle: string; questionLabel: string; formId: string; questionId: string }[];
  onCancel: () => void;
  onConfirm: () => void;
  t: TFunction;
}
...
interface AlternativeWarningModalProps {
  warning: { title: string; message: string };
  onClose: () => void;
  t: TFunction;
}

const AlternativeWarningModal = ({
  warning,
  onClose,
  t
}: AlternativeWarningModalProps) => {
  return (
...
const DeleteFormConfirmModal = ({
  formToDelete,
  onCancel,
  onConfirm,
  t
}: DeleteFormConfirmModalProps) => {
  return (
    <div className="modal-overlay" onClick={onCancel}>
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        <div className="modal-card-header">
...
const isDuplicateName = (forms: Form[], title: string, formId?: string) => {
...
const isDuplicateQuestionLabel = (selectedForm: Form | undefined, label: string, questionId: string) => {
  if (!selectedForm) return false;
  return selectedForm.questions.some(q => q.id !== questionId && q.label.toLowerCase().trim() === label.toLowerCase().trim());
};

const getUniqueQuestionLabel = (selectedForm: Form | undefined, baseLabel: string) => {
...
const questionHasInformation = (question: FormQuestion, newQuestionLabel: string) => {
  const hasCustomLabel = question.label.trim() !== '' && question.label.trim() !== newQuestionLabel.trim();
...
const evaluateQuestionDependency = (question: FormQuestion, previewAnswers: Record<string, string | string[] | number | undefined>) => {
...
const getDependentQuestions = (forms: Form[], targetQuestionId: string) => {
...
const computeUnlinkedQuestions = (questions: FormQuestion[], targetQuestionId: string) => {
  return questions.map(q => {
    if (q.dependencyQuestion === targetQuestionId) {
      const rest = { ...q };
      delete rest.dependencyQuestion;
      delete rest.dependencyCondition;
      return rest;
    }
    return q;
  });
...
export const FormLibrary = () => {
  ...
  const forms = React.useMemo(() => workflow.forms || [], [workflow.forms]);
  ...
  const [previewAnswers, setPreviewAnswers] = useState<Record<string, string | string[] | number | undefined>>({});
  ...
  const handleCreateForm = () => {
    ...
    let counter = 1;
    while (isDuplicateName(forms, newTitle)) {
  ...
  const handleTitleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isDuplicateName(forms, newTitle, selectedForm.id) && newTitle.trim() !== '') {
  ...
  const handleAddQuestion = () => {
    ...
    const newQuestion: FormQuestion = {
      id: `q-${crypto.randomUUID()}`,
      type: 'text',
      label: getUniqueQuestionLabel(selectedForm, t('forms.new_question')),
      required: false
    };
  ...
  const handleAddDisclaimer = () => {
    ...
    const newDisclaimer: FormQuestion = {
      id: `q-${crypto.randomUUID()}`,
      type: 'disclaimer',
      label: getUniqueQuestionLabel(selectedForm, t('forms.new_disclaimer_title')),
  ...
  const deleteQuestionById = (questionId: string) => {
    ...
    const deps = getDependentQuestions(forms, questionId);
    ...
    Object.entries(formUpdates).forEach(([depFormId, depQuestionIds]) => {
      ...
      if (f) {
        const unlinked = f.questions.map(q => {
        ...
        updateForm(depFormId, { questions: assignDisplayNumbers(unlinked) });
      }
    });

    const filtered = selectedForm.questions.filter(q => q.id !== questionId);
    const unlinkedSelected = computeUnlinkedQuestions(filtered, questionId);

    updateForm(selectedForm.id, {
      questions: assignDisplayNumbers(unlinkedSelected)
    });
  };
  ...
  const handleDeleteQuestion = (questionId: string) => {
    ...
    if (deps.length > 0 || questionHasInformation(question, t('forms.new_question'))) {
    ...
    <div className="form-library-layout">
                ...
                {!selectedForm ? (
                  ...
                  <div className="editor-form">
                    <div className="editor-section config-card">
                      ...
                      <div className="field-group">
                        <label style={{ fontWeight: 'bold', marginBottom: 'var(--spacing-sm)' }}>
                          {t('forms.form_title_label')}
                        </label>
                        <input
                          type="text"
                          className="form-input"
                      ...
                      {ownerTask && (
                        <div style={{
                          ...
                          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--primary, #2563eb)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
                        ...
                        <div ref={questionsListRef} className="questions-editor-list">
                          {selectedForm.questions.map((q, index) => (
                            <QuestionEditorCard
                              ...
                              handleDeleteQuestion={handleDeleteQuestion}
                              handleMoveQuestion={handleMoveQuestion}
                              isDuplicateQuestionLabel={(label, qId) => isDuplicateQuestionLabel(selectedForm, label, qId)}
                              handleQuestionUpdate={handleQuestionUpdate}
                              setAlternativeWarning={setAlternativeWarning}
                              t={t}
                            />
                      ...
                      <div className="preview-form">
                        {selectedForm.questions.length === 0 ? (
                          ...
                          selectedForm.questions
                            .filter(q => evaluateQuestionDependency(q, previewAnswers))
                            .map(q => (
                              <QuestionPreviewItem
                                key={q.id}
                                q={q}
      ...
      {questionToDelete && (
        <DeleteQuestionConfirmModal
          ...
          questionToDeleteDeps={questionToDeleteDeps}
          onCancel={() => { setQuestionToDelete(null); setQuestionToDeleteDeps([]); }}
      ...
      {alternativeWarning && (
        <AlternativeWarningModal
          warning={alternativeWarning}
          onClose={() => setAlternativeWarning(null)}
          t={t}
        />
      )}
